    ...
    if let Ok(lines) = util::read_lines("resources\\in7ex.txt") {
        for line in lines {
            if let Ok(ip) = line {
                ...
                if ip.starts_with('$') {
                    ...
                    match command {
                        "cd" => {
                            ...
                            directory_list.push(FileOrFolder{is_file: false, size: 0, name: folder.to_string(), files: Vec::new()});
                ...
                } else if ip.starts_with("dir") {
                    let (_, folder) = ip.split_once(' ').unwrap();
                    println!("Folder named: {}", folder);
                } else if ip.starts_with(char::is_numeric) {
                    let (size, file) = ip.split_once(' ').unwrap();
