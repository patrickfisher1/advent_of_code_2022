...
pub(crate) fn part1() {
    ...
    if let Ok(lines) = util::read_lines("resources\\in4.txt") {
        ...
        for line in lines.flatten() {
            ...
            let (elf1_range_start, elf1_range_end) = (
                split.next().unwrap().parse::<i32>().unwrap(),
                split.next().unwrap().parse::<i32>().unwrap(),
            );

            let mut split = elf2.split('-');
            let (elf2_range_start, elf2_range_end) = (
                split.next().unwrap().parse::<i32>().unwrap(),
                split.next().unwrap().parse::<i32>().unwrap(),
            );

            if elf1_range_start <= elf2_range_start && elf1_range_end >= elf2_range_end
                || elf2_range_start <= elf1_range_start && elf2_range_end >= elf1_range_end
            {
                contained_pairs += 1;
            }
        }
...
pub(crate) fn part2() {
    ...
    if let Ok(lines) = util::read_lines("resources\\in4.txt") {
        ...
        for line in lines.flatten() {
            ...
            let elf2_range = elf2_range_start..elf2_range_end + 1;
